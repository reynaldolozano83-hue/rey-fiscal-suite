...
def verify_premium_plan(org_id: int):
    ...
    if not org or org['plan_type'] not in ['STARTER', 'CRECIMIENTO', 'ESCALA', 'PREMIUM'] or org['subscription_status'] != 'Active':

        raise HTTPException(

            status_code=403,

            detail="ACCESO DENEGADO: El modulo Premium (Invoicing/Payroll/Inventory) requiere suscripcion activa."
...
def get_inventory(org_id: int = 1):

    conn = database.get_connection()

    cursor = conn.cursor()

    cursor.execute("SELECT * FROM inventarios WHERE organization_id = ?", (org_id,))
...
def add_inventory_product(req: ProductCreateRequest, org_id: int = 1):
    ...
    cursor.execute("""
        ...
        INSERT INTO inventarios (organization_id, nombre, cantidad, unidad, fecha_actualizacion)

        VALUES (?, ?, ?, ?, ?)

    """, (org_id, req.nombre, req.cantidad, req.unidad, now_str))
...
def register_inventory_movement(req: MovementRequest, org_id: int = 1):

    conn = database.get_connection()

    cursor = conn.cursor()

    cursor.execute("SELECT cantidad FROM inventarios WHERE id = ? AND organization_id = ?", (req.product_id, org_id))
    ...
    cursor.execute("""
        ...
        UPDATE inventarios SET cantidad = ?, fecha_actualizacion = ? WHERE id = ?

    """, (new_qty, now_str, req.product_id))
...
def add_employee(req: EmployeeCreateRequest, org_id: int = 1):
    ...
    cursor.execute("SELECT plan_type FROM organizations WHERE id = ?", (org_id,))
    ...
    plan_limits = {"FREE": 1, "STARTER": 10, "CRECIMIENTO": 35, "ESCALA": 100, "PREMIUM": 500}

    limit = plan_limits.get(plan, 1)

    

    cursor.execute("SELECT COUNT(*) FROM empleados WHERE organization_id = ?", (org_id,))
    ...
    if current_count >= limit:
        ...
        raise HTTPException(status_code=403, detail=f"Límite de empleados alcanzado (Máx. {limit}).")
...
class PayrollCalcRequest(BaseModel):
    ...
    pct_fondo_ahorro: float = 10.0

    is_aguinaldo_anual: bool = False

    conceptos_adicionales: Optional[dict] = None
    ...
    pct_prima_vacacional: float = 25.0

    is_prima_vacacional_aniversario: bool = False
...
def calculate_payroll(req: PayrollCalcRequest, org_id: int = 1):
    ...
    if req.is_prima_vacacional_aniversario:

        from datetime import date

        f_ingreso = datetime.strptime(emp['fecha_ingreso'], "%Y-%m-%d").date()

        today = date.today()

        antiguedad_anos = (today - f_ingreso).days / 365.0
        ...
        monto_vacaciones = v_days * emp['salario_diario']

        monto_prima = monto_vacaciones * (req.pct_prima_vacacional / 100.0)

        

        desglose = {

            "vacaciones_dias": v_days,

            "monto_vacaciones": round(monto_vacaciones, 2),

            "pct_prima": req.pct_prima_vacacional,

            "monto_prima": round(monto_prima, 2),

            "neto_pagar": round(monto_prima, 2)

        }

        total_neto = monto_prima

        tipo_calc = 'PrimaVacacional'

        

    elif req.is_aguinaldo_anual:
        ...
        f_inicio_ano = date(today.year, 1, 1)

        dias_trabajados_ano = (today - max(f_ingreso, f_inicio_ano)).days + 1

        dias_trabajados_ano = min(dias_trabajados_ano, 365)

        

        dias_aguinaldo = req.dias_aguinaldo

        prop_aguinaldo = (dias_trabajados_ano / 365.0) * dias_aguinaldo * emp['salario_diario']

        

        desglose = {

            "dias_trabajados_ano": dias_trabajados_ano,

            "dias_aguinaldo": dias_aguinaldo,

            "prop_aguinaldo": round(prop_aguinaldo, 2),

            "neto_pagar": round(prop_aguinaldo, 2)

        }

        total_neto = prop_aguinaldo
        ...
        desglose = payroll.calculate_finiquito_liquidacion(
            ...
            pct_prima_vacacional=req.pct_prima_vacacional
        ...
        calc = payroll.calculate_payroll_receipt(
            ...
            pct_fondo=req.pct_fondo_ahorro,

            horas_dobles=req.horas_dobles,

            horas_triples=req.horas_triples,

            conceptos_adicionales=req.conceptos_adicionales,

            infonavit=req.infonavit,
