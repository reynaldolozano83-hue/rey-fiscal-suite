...
async function loadInventory() {
    try {
        const res = await fetch(`${API_BASE}/inventory?org_id=${activeOrgId}`);
        const data = await res.json();
        ...
        if (data.length === 0) {
            tbody.innerHTML = `<tr><td colspan="5" class="py-8 text-center text-slate-500 italic">No hay productos registrados en el inventario.</td></tr>`;
            return;
        }
        
        data.forEach(p => {
            const row = document.createElement("tr");
            row.className = "border-b border-slate-800 hover:bg-slate-900/40 transition";
            row.innerHTML = `
                <td class="py-3 px-4 text-white font-bold text-xs">${p.nombre}</td>
                <td class="py-3 px-4 text-right text-cyanNeon font-bold font-mono">${p.cantidad.toFixed(1)}</td>
                <td class="py-3 px-4 text-xs font-mono text-slate-400">${p.unidad}</td>
                <td class="py-3 px-4 text-[10px] text-slate-500 font-mono">${p.fecha_actualizacion || 'Sin movimientos'}</td>
                <td class="py-3 px-4 text-center">
                    <div class="flex justify-center gap-2">
                        <button onclick="registerMovement(${p.id}, 10)" class="px-2 py-0.5 bg-greenNeon/10 hover:bg-greenNeon/25 text-greenNeon text-[9px] font-bold rounded border border-greenNeon/25 transition cursor-pointer">+ 10</button>
                        <button onclick="registerMovement(${p.id}, -10)" class="px-2 py-0.5 bg-rose-500/10 hover:bg-rose-500/25 text-rose-400 text-[9px] font-bold rounded border border-rose-500/25 transition cursor-pointer">- 10</button>
                        <button onclick="promptCustomMovement(${p.id}, '${p.nombre}', '${p.unidad}')" class="px-2 py-0.5 bg-slate-800 hover:bg-slate-700 text-slate-300 text-[9px] font-bold rounded border border-slate-700 transition cursor-pointer">Otro</button>
                    </div>
                </td>
            `;
...
function closeAddProductModal() {
    ...
    document.getElementById("prod-new-qty").value = "0";
}

async function submitNewProduct() {
    const nombre = document.getElementById("prod-new-name").value.trim();
    const unidad = document.getElementById("prod-new-unit").value;
    const qty = parseFloat(document.getElementById("prod-new-qty").value) || 0.0;
    ...
    try {
        const res = await fetch(`${API_BASE}/inventory?org_id=${activeOrgId}`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ nombre: nombre, unidad: unidad, cantidad: qty })
        });
        const data = await res.json();
        if (data.status === 'success') {
...
async function registerMovement(productId, amount) {
    try {
        const res = await fetch(`${API_BASE}/inventory/movement?org_id=${activeOrgId}`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ product_id: productId, cantidad: amount })
        });
        const data = await res.json();
        if (data.status === 'success') {
            await loadInventory();
...
function promptCustomMovement(productId, name, unit) {
    const qtyStr = prompt(`Registrar movimiento para ${name}:\n(Usa número positivo para entradas y negativo para salidas/mermas en ${unit})`);
    if (!qtyStr) return;
    ...
    registerMovement(productId, qty);
}
